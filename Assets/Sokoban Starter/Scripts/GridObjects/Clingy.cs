...
public class Clingy : BetterGridObject
{
    ...
    public override bool PassiveMove(Vector2Int sourcePosition, Vector2Int moveAmount)
    {
        ...
        if (realPosition + moveAmount != sourcePosition)
            return false;
        
        var newPos = sourcePosition;
        var target = GameManager.reference.gridObjects[sourcePosition.y][sourcePosition.x];
        if (target != null)
        {
            if (!(target as Player))
                if (target.PassiveMove(realPosition, moveAmount))
                    return false;
        }
